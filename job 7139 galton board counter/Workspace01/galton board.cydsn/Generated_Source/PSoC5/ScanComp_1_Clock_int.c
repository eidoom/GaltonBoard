/*******************************************************************************
* File Name: ScanComp_1_Clock_int.c
* Version 2.20
*
*  Description:
...
#include "ScanComp_1_Clock_int.h"
...
/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_Start
********************************************************************************
*
* Summary:
...
void ScanComp_1_Clock_int_Start(void) 
{
    ...
    ScanComp_1_Clock_int_CLKEN |= ScanComp_1_Clock_int_CLKEN_MASK;
	ScanComp_1_Clock_int_CLKSTBY |= ScanComp_1_Clock_int_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_Stop
********************************************************************************
*
* Summary:
...
void ScanComp_1_Clock_int_Stop(void) 
{
    ...
    ScanComp_1_Clock_int_CLKEN &= (uint8)(~ScanComp_1_Clock_int_CLKEN_MASK);
	ScanComp_1_Clock_int_CLKSTBY &= (uint8)(~ScanComp_1_Clock_int_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ScanComp_1_Clock_int_CLKEN & ScanComp_1_Clock_int_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ScanComp_1_Clock_int_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ScanComp_1_Clock_int_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ScanComp_1_Clock_int__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ScanComp_1_Clock_int_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ScanComp_1_Clock_int_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ScanComp_1_Clock_int_GetSourceRegister();
    uint16 oldDivider = ScanComp_1_Clock_int_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ScanComp_1_Clock_int_CLKEN & ScanComp_1_Clock_int_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ScanComp_1_Clock_int_DIV_PTR, clkDivider);
                ScanComp_1_Clock_int_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ScanComp_1_Clock_int_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ScanComp_1_Clock_int_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ScanComp_1_Clock_int_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ScanComp_1_Clock_int_DIV_PTR, clkDivider);
				ScanComp_1_Clock_int_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ScanComp_1_Clock_int_DIV_PTR);
}


/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ScanComp_1_Clock_int_MOD_SRC |= modeBitMask & (uint8)ScanComp_1_Clock_int_MODE_MASK;
}


/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ScanComp_1_Clock_int_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ScanComp_1_Clock_int_MODE_MASK));
}


/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ScanComp_1_Clock_int_MOD_SRC & (uint8)(ScanComp_1_Clock_int_MODE_MASK);
}


/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ScanComp_1_Clock_int_GetDividerRegister();
    uint8 oldSrc = ScanComp_1_Clock_int_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ScanComp_1_Clock_int_MOD_SRC |= CYCLK_SSS;
        ScanComp_1_Clock_int_MOD_SRC =
            (ScanComp_1_Clock_int_MOD_SRC & (uint8)(~ScanComp_1_Clock_int_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ScanComp_1_Clock_int_MOD_SRC & ScanComp_1_Clock_int_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ScanComp_1_Clock_int_PHASE = clkPhase & ScanComp_1_Clock_int_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ScanComp_1_Clock_int_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ScanComp_1_Clock_int_PHASE & ScanComp_1_Clock_int_PHASE_MASK;
}
